...
void EpollPoller::updateChannel(Channel *channel)
{
    ...
    LOG_DEBUG("fd = %d , events = %d , index = %d\n",channel->fd(),channel->events(),index);
...
void EpollPoller::update(int operation,Channel* channel)
{
    ...
    int fd = channel->fd();
    LOG_DEBUG("fd = %d is to be epolled or deleted in loop %p on thread %d",fd,getOwnerLoop(),CurrentThread::gettid());
...
{
    LOG_DEBUG("fd to be monitored total count : %lu in loop %p on thread %d\n",channels_.size(),getOwnerLoop(),CurrentThread::gettid());
