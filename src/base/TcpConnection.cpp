      ...
      highWaterMark_(64*1024*1024)      //  64 MB
{
    ...
    LOG_DEBUG("TcpConnection::ctor [%s] on fd = %d\n",name_.c_str(),connfd);
    socket_->setKeepAlive(true);
}

TcpConnection::~TcpConnection()
{
    LOG_DEBUG("TcpConnection::dector (%p) [%s] on socket %d with state %d\n",this,name_.c_str(),socket_->fd(),state_.load());
}
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    if(n>0)
    {
        ...
        {
            //  user没设置tcpConnection 如何处理收到的数据
            //  那么数据量会一直不为0，由于epoll设置为水平触发，则会不断触发、server陷入死循环.
            inputBuffer_.retrieveAllAsString();
            LOG_DEBUG("messageCallback is empty\n");
        }   
    }
    else if(n == 0)
    {
...
void TcpConnection::handleWrite()
{
    ...
    if(channel_->isWriting())
    {
        ...
        if(n > 0)
        {
            ...
            {
                LOG_DEBUG("I'm going to write more data");
            }
        }
        else
        {
...
void TcpConnection::handleClose()
{
    ...
    // LOG_DEBUG("shared_ptr cnt %ld",shared_from_this().use_count());
    TcpConnectionPtr connPtr(shared_from_this());
    // LOG_DEBUG("shared_ptr cnt %ld",connPtr.use_count());
    
    //  user设置的
    if(connectionCallback_)
        connectionCallback_(connPtr);       //  连接断开和连接的回调
    else    
        LOG_DEBUG("connectionCallback is empty\n");

    //  不是user设置的
    //  TcpServer::removeConnection -> TcpConnection::connectionDestroyed
    closeCallback_(connPtr);            //  关闭连接的回调  TcpServer::removeConnection

    // LOG_DEBUG("shared_ptr cnt %ld tcpconnection %p point to %p",connPtr.use_count(),connPtr.get(),this);
}
...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    if(!faultError && remaning > 0)
    {
        ...
        if(!channel_->isWriting())
        {
            LOG_DEBUG("将fd = %d的写事件 注册给poller",channel_->fd());
...
void TcpConnection::connectionEstablished()
{
    ...
    {
        LOG_DEBUG("user didn't set connectionCallback ; connectionCallback is nullptr");
    }
}
...
{
    LOG_DEBUG("state_ = %d",state_.load());
