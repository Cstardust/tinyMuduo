...
public:
    ...
    ParseResult parse(HttpRequest &req, const char *begin, const char *end)
    {
...
private:
    ...
    ParseResult consume(HttpRequest &req, const char *begin, const char *end)
    {
        while( begin != end )
        {
            char input = *begin++;
            ...
            case HeaderLineStart:
                ...
                {
                    req.headers.push_back(HttpRequest::HeaderItem());
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    ...
                    {
                        HttpRequest::HeaderItem &h = req.headers.back();

                        if( strcasecmp(h.name.c_str(), "Content-Length") == 0 )
                        {
                            contentSize = atoi(h.value.c_str());
			    req.setContentLen(contentSize);
            ...
            case ExpectingNewline_3: {
                std::vector<HttpRequest::HeaderItem>::iterator it = std::find_if(req.headers.begin(),
                                                                    req.headers.end(),
                                                                    checkIfConnection);
