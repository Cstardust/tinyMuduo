...
public:
    ...
    ParseResult parse(HttpResponse &resp, const char *begin, const char *end)
    {
        return consume(resp, begin, end);
    }

private:
    static bool checkIfConnection(const HttpResponse::HeaderItem &item)
    {
        return strcasecmp(item.name.c_str(), "Connection") == 0;
    }

    ParseResult consume(HttpResponse &resp, const char *begin, const char *end)
    {
        while( begin != end )
        {
            char input = *begin++;
            ...
            case HeaderLineStart:
                ...
                {
                    resp.headers.push_back(HttpResponse::HeaderItem());
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    HttpResponse::HeaderItem &h = resp.headers.back();
            ...
            case ExpectingNewline_3: {
                std::vector<HttpResponse::HeaderItem>::iterator it = std::find_if(resp.headers.begin(),
                                                                    resp.headers.end(),
                                                                    checkIfConnection);
